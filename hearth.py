...
# Constants
DECKS_PER_PAGE = 25.0
# Not sure what this value did in first place. This value WAS 0.1
PAGE_MULTIPLICATOR = 1.0
...
def get_decks_per_class(filtering=None, sorting=None, count=None, patch=None):
    ...
    if not count:
        ...
        count = int((pagecount * DECKS_PER_PAGE * PAGE_MULTIPLICATOR) / len(classes))
...
def get_decks(filtering=None, sorting=None, count=None, patch=None, classid=None):
    ...
    print(decks_metainfo[-1])
    sys.stdout.write("Going to read "+str(len(decks_metainfo))+" Decks\n")
...
def get_deck_list(deckid):
    ...
    """
    sys.stdout.write("Parsing Deck: "+str(deckid)+"\n")
    ...
    for card in cards:
        ...
        if match:
            ...
            cardname = bytes(cardname).decode(encoding='UTF-8')
            #print(cardname)
...
def get_deck_metainfo(filtering=None, sorting=None, count=None, patch=None, classid=None):
    ...
    if not count:
        pagecount = get_pagecount(get_pagetree(url))
        count = int(pagecount * PAGE_MULTIPLICATOR)
    ...
    for pagenum in range(1, int(pagecount)+1):  # Adding one as range is exclusive
        sys.stdout.write("Reading Deckpage "+str(pagenum)+" of "+str(pagecount)+"\n")
...
def get_cards():
    ...
    with open("mashape_key.txt", "r") as mashape_key:
        api_key = mashape_key.read().encode('utf-8')
    api_key = bytes(api_key).decode(encoding='UTF-8')
...
def populate_card_db(cards, cursor):
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS cards
                      (cardname text, cardset text,
                       hero text, rarity text, cost integer,
                       printed_text text,
